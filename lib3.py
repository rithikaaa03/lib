...
class LibraryManagementSystem:
    def __init__(self,root):
        ...
        self.cnx = mysql.connector.connect(user='root', password='root',host='localhost', database='mydata2')
        ...
        btnAddData=Button(Framebutton,text="Add book",font=("arial",12,"bold"),width=11,bg="blue",fg="white")
    ...
    def adda_data(self):
        ...
        conn=mysql.connector.connect(host="localhost",username="root",password="root",database="mydata2")
        my_cursor=conn.cursor()
        my_cursor.execute("insert into book values(%s,%s,%s,%s)",
                          (self.bookid_var.get(),self.booktitle_var.get(),self.author_var.get(),
    ...
    def update(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="root",database="mydata2")
        my_cursor=conn.cursor()
        my_cursor.execute("update person set Member=%s,FirstName=%s,LName=%s,Address1=%s,Address2=%s,Mobile=%s where idno=%s",
    ...
    """def update(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="root",database="mydata2")
        my_cursor=conn.cursor()
        my_cursor.execute("update book set  where bookid=%s",
                          (self.member_var.get(),self.firstname_var.get(),self.lastname_var.get(),
                           self.address1_var.get(),self.address2_var.get(),self.mobile_var.get(),self.id_var.get()))
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="root",database="mydata2")
        my_cursor=conn.cursor()
        my_cursor.execute("select member,idno,firstname,lname,address1,address2,mobile,bookid,booktitle,author,borrowdate,duedate,returndate,finalprice from person natural join book natural join borrow")
    ...
    def showData(self):
        ...
        try:
            ...
            book_id = self.bookid_var.get()
            cursor.callproc('daysonbook', [book_id])
        # print results
            result = self.cursor.fetchone()[0]
            self.txtBox.insert(END,"Late Return Fine:\t\t"+result+"\n")
        ...
        """book_id = self.bookid_var.get()
        # call daysonbook function with book_id parameter
        self.cursor.callproc('daysonbook', [book_id])
        # fetch the result from the cursor
        result = self.cursor.fetchone()[0]
        self.txtBox.insert(END,"Days on Book:\t\t"+result+"\n")
        self.cursor.callproc('latereturnfee', [book_id])
        result = self.cursor.fetchone()[0]
        self.txtBox.insert(END,"Late Return Fine:\t\t"+result+"\n")"""
