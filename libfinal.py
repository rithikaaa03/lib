...
class LibraryManagementSystem:
    ...
    def showData(self):
        ...
        daysquery = """Call daysonbook1(%s);"""
        daysarg = (book_id,)
        cursor.execute(daysquery,daysarg)
        result   = cursor.fetchall()
        self.txtBox.insert(END,"days on book:\t\t"+str(result[0][0])+"\n")
        cursor.close()
        connection.close()
        ...
        latequery = """Call latereturnfee1(%s);"""
        latearg = (book_id,)
        cursor1.execute(latequery,latearg)
        result1   = cursor1.fetchone()[0]
        self.txtBox.insert(END,"late return fee:\t\t"+str(result1)+"\n")
        cursor1.close()
        connection1.close()
